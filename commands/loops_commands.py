...
from commands.key_manager import get_server_key
...
def register_loop_commands(bot):
    """Register all loop commands and their stop counterparts"""
    ...
    async def drag(interaction: discord.Interaction, member: discord.Member, vc1: discord.VoiceChannel, vc2: discord.VoiceChannel, times: int):
        ...
        if times < 1 or times > 99999:
            await interaction.response.send_message("Please provide a valid number of times (between 1 and 99999).", ephemeral=True)
            return
    ...
    @app_commands.describe(
        ...
        key="Authorization key for command"
    )
    async def dm_loop(
        ...
        # Get the server-specific key
        server_key = get_server_key(interaction.guild.id)
    ...
    async def stop_dm(interaction: discord.Interaction, username: discord.Member, key: str):
        # Get the server-specific key
        server_key = get_server_key(interaction.guild.id)
        ...
        if member_id in active_tasks["dm"]:
            active_tasks["dm"][member_id].cancel()
            active_tasks["dm"].pop(member_id, None)
            await interaction.response.send_message(f"Stopped DM loop for {username.mention}!", ephemeral=True)
        else:
            await interaction.response.send_message(f"No active DM loop found for {username.mention}.", ephemeral=True)
    ...
    async def stop_tag(interaction: discord.Interaction, username: discord.Member, key: str):
        # Get the server-specific key
        server_key = get_server_key(interaction.guild.id)
        ...
        if member_id in active_tasks["tag"]:
            active_tasks["tag"][member_id].cancel()
            active_tasks["tag"].pop(member_id, None)
            await interaction.response.send_message(f"Stopped tag loop for {username.mention}!", ephemeral=True)
        else:
            await interaction.response.send_message(f"No active tag loop found for {username.mention}.", ephemeral=True)